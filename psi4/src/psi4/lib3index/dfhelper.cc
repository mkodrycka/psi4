...
#pragma omp parallel num_threads(nthreads_)
    ...
    std::unique_ptr<double[]> M(new double[std::get<0>(plargest) / 2]);  
...
#pragma omp parallel for schedule(guided) num_threads(nthreads_)
        ...
        for (size_t k = 0; k < nbf_; k++) {
            for (size_t m = k + 1, count = 0; m < nbf_; m++) {
                if (schwarz_fun_mask_[k * nbf_ + m]) {
                    count++;
...
void DFHelper::set_do_JK_tt(bool do_JK_tt) {
    std::stringstream error;
    error << "DFHelper Base class doesn't do assymetric JK's. Please use DFHelper_2B.";
    throw PSIEXCEPTION(error.str().c_str());
}
