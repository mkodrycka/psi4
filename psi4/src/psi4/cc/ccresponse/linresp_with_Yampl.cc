...
double YC(const char *pert_y, int irrep_y, double omega_y, const char *pert_c, int irrep_c, double omega);
...
void linresp_with_Yampl(double *tensor, double A, double B, const char *pert_x, int x_irrep, double omega_x, const char *pert_y,
             int y_irrep, double omega_y) {
    double polar, polar_YC, polar_LCX, polar_HXY, polar_LHX1Y1, polar_LHX1Y2, polar_LHX2Y2;
    ...
    if ((x_irrep ^ y_irrep) == 0) {
        if (omega_y != 0.0) { 
            ...
            polar_YC = YC(pert_y, y_irrep, omega_y, pert_x, x_irrep, omega_x);
            //polar_LCX = LCX(pert_x, x_irrep, pert_y, y_irrep, omega_y);
            polar_LCX = LCX_with_Y(pert_y, y_irrep, pert_x, x_irrep, omega_x);
        ...
        if (params.sekino || params.linear) /* only linear term needed in Sekino-Bartlett model III */
            polar = polar_LCX;
             ...
             polar = polar_YC + polar_LCX;
            //polar = polar_YC + polar_LCX + polar_HXY + polar_LHX1Y1 + polar_LHX2Y2 + polar_LHX1Y2;
